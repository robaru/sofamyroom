		...
		for (si = 0; si < arg->pSetup->nSources; si++)
		{
			...
			for (ri = 0; ri < arg->pSetup->nReceivers; ri++)
			{
				...
				if (receiverimpulse)
				{
					...
					for (ch = 1; ch < arg->pSimulation[r].receiver[ri].definition->nChannels; ch++)
					{
						Conv(h + hlen * ch, hlen, x, xlen, y + ylen * ch);
						nChannels++;
					}
				...
				for (ch = 1; ch < arg->pSimulation[r].receiver[ri].definition->nChannels; ch++)
					for (i = 0; i < lim; i++)
						arg->pSimulation[r].brir[sr].sample[arg->pSimulation[r].brir[sr].nSamples * ch + ofs + i] += x[xlen * ch + i];
...
CRoomsimInternal *RoomsimInit(const CRoomSetup *pSetup, sfmt_t *sfmt)
{
	...
	for (currentRoom = 0; currentRoom < pSetup->nRooms; currentRoom++)
	{
		...
		/* allocate memory for convolution results */
		{
			...
			for (r = 0; r < pSimulation[currentRoom].nReceivers; r++)
				if (maxrlen > 0) { len += maxrlen; total += len * pSimulation[currentRoom].receiver[r].definition->nChannels; }
		...
		/* if uncorrelated, no need to check for channel number */
		for (r = 0; r < pSimulation[currentRoom].nReceivers; r++)
		{
			if (pSetup[currentRoom].options.uncorrelatednoise)
			{
				pSimulation[currentRoom].noise = MemMalloc(((pSimulation[currentRoom].receiver[r].definition->nChannels * length + 3) & -4) * sizeof(unsigned int));
				pSimulation[currentRoom].shapednoise = MemMalloc(pSimulation[currentRoom].receiver[r].definition->nChannels * length * sizeof(double));
				pSimulation[currentRoom].directionalshapednoise = MemMalloc(pSimulation[currentRoom].receiver[r].definition->nChannels * length * sizeof(double));
			}
			else
			{
				pSimulation[currentRoom].noise = MemMalloc(((length + 3) & -4) * sizeof(unsigned int));
				pSimulation[currentRoom].shapednoise = MemMalloc(length * sizeof(double));
				pSimulation[currentRoom].directionalshapednoise = MemMalloc(length * sizeof(double));
			}
		}
...
void RoomsimDiffuse(const CRoomSetup *pSetup, CRoomsimInternal *pSimulation, sfmt_t *sfmt)
{
	...
	/* loop counters */
	int	iSource, iBand, iRay, iReceiver, iRoom, iChannel;
...
#ifdef LOGTAIL
			{
				...
				for (iReceiver = 0; iReceiver < pSetup->nReceivers; iReceiver++)
				{
					...
					for (iDirection = 0; iDirection < 6; iDirection++)
					{
						...
						{
							for (iChannel = 1; iChannel < pSimulation[iRoom].receiver[iReceiver].definition->nChannels; iChannel++)
							/* apply time-varying filter to second channel of noise signal */
							TimeVaryingConv(pSimulation[iRoom].htv, NFFT_SIZE,
								pSimulation[iRoom].htvidx, nTimebin,
								pSimulation[iRoom].noise + length * iChannel, length,
								ROUND(pSimulation[iRoom].receiver[iReceiver].FirstTOA[iDirection] * pSimulation[iRoom].fs),
								noisethreshold, pSimulation[iRoom].shapednoise + length * iChannel);
						}
						...
						case SR_LOGGAIN:
							...
							for (i = 0; i < pSimulation[iRoom].receiver[iReceiver].definition->nChannels * length; i++)
						...
						if (receiverimpulse)
						{
							...
							for (iChannel = 1; iChannel < pSimulation[iRoom].receiver[iReceiver].definition->nChannels; iChannel++)
							{
								...
								if (pSetup->options.uncorrelatednoise)
								{
									FIRfilter(
										receiverimpulse, receiverimpulselength,			/* filter */
										pSimulation[iRoom].shapednoise + length * iChannel, length,	    /* signal */
										pSimulation[iRoom].directionalshapednoise + length * iChannel,	/* output */
										NULL											/* state */
									);
								}
								else
							...
							length *= pSimulation[iRoom].receiver[iReceiver].definition->nChannels;
							//}
						}
