        ...
        int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs==1 && mxIsStruct(prhs[0]))
    {
        ...
        brir_pointer = Roomsim(&roomsetup);
        ...
        if (roomsetup.options.mex_saveaswav)
        {
            ...
            sample = (float*)malloc(*(brir_pointer)[0].nChannels * sizeof(float));
            ...
            {
                for (int j = 0; j < roomsetup.nRooms; j++)
                {
                    BRIR *brir = *(brir_pointer);
            ...
            if (nlhs>0)
            {
                ...
                if ((roomsetup.nRooms == 1) && (roomsetup.nSources == 1) && (roomsetup.nReceivers == 1))
                {
                    BRIR *brir = *(brir_pointer);
